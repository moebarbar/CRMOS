      ...
      <div className="module-head">
        <div>
          ...
          <div className="module-sub">
            Monday, April 27 · You have 3 things on your plate today.
          </div>
        </div>
        <div className="module-toolbar">
          ...
          <button className="btn primary sm">
            <Icon name="sparkle" size={14} /> Ask Moe
          </button>
        </div>
      </div>
      <div className="module-content">
        <div className="stats-grid" style={{ marginBottom: 16 }}>
          <div className="stat">
            ...
            <div className="stat-delta up">
              <Icon name="arrowUp" size={12} /> +$8.4k this week
            </div>
          </div>
          <div className="stat">
          ...
          <div className="stat">
            ...
            <div className="stat-delta up">
              <Icon name="play" size={11} /> Acme · running
            </div>
          </div>
        </div>
        ...
        <div className="today-grid">
          <div className="today-card">
            <h3>
              <Icon name="calendar" size={14} /> Today's schedule{' '}
              <span className="tag mono">6 BLOCKS</span>
            </h3>
                  ...
                  <div className={`timeline-event ${t.type === 'focus' ? 'focus' : ''}`}>
                    ...
                    <div className="timeline-event-meta">
                      ...
                      {t.client && (
                        <>
                          <span>·</span>
                          <span>{clientById(t.client).name}</span>
                        </>
                      )}
                      {t.team && (
                        <span className="avatar-stack" style={{ marginLeft: 'auto' }}>
                          {t.team.map((u) => (
                            <Avatar
                              key={u}
                              name={userById(u).name}
                              size={20}
                              color={userById(u).color}
                            />
                          ))}
                        </span>
                      )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className="today-card">
              <h3>
                <MoeIcon size={16} state="speaking" /> Moe's morning brief
              </h3>
              <div
                style={{
                  display: 'flex',
              ...
              >
                <div>
                  ...
                  <strong>Northwind</strong> is 11d overdue — chase drafted in your tone.
                </div>
                <div>
                  <span className="dot" /> <strong>Acme</strong> signed the proposal at 11:42 PM.
                  Project + 12 tasks created.
                </div>
                <div>
                  <span className="dot" style={{ background: '#fbbf24' }} />{' '}
                  <strong>Bywater</strong> viewed your proposal 6 times — high intent.
                </div>
                <div>
                  <span className="dot" style={{ background: '#60a5fa' }} /> 2 new leads from forms
                  overnight.
                </div>
              </div>
              <button
              ...
              >
                Review the 3 chases <Icon name="arrow" size={12} />
              </button>
            </div>
            <div className="today-card">
              <h3>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {[
                  ...
                  <button key={i} className="btn sm" style={{ justifyContent: 'flex-start' }}>
...
const InboxModule = () => {
  ...
  const filtered =
    filter === 'all' ? D.INBOX_MESSAGES : D.INBOX_MESSAGES.filter((m) => m.channel === filter);
  return (
      ...
      <div className="inbox-list">
        ...
        {filtered.map((m) => (
          <div
            ...
            className={`inbox-msg ${m.unread ? 'unread' : ''} ${selected?.id === m.id ? 'active' : ''}`}
          >
          ...
          <>
            <div className="inbox-detail-head">
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                <span className={`channel-badge channel-${selected.channel}`}>
                  {selected.channel}
                </span>
            ...
            {selected.ai && (
              <div className="inbox-ai-summary">
                ...
                <div>
                  <strong>Moe summary:</strong> {selected.preview} I drafted a reply matching your
                  tone — review below.
                </div>
              </div>
            )}
            ...
            <div className="reply-box">
              <textarea
                defaultValue={
                  selected.ai
                    ? `Hey ${selected.name.split(' ')[0]} — sounds great. We can kick off Monday with a 30-min sync, then start the discovery sprint Tuesday. I'll send a calendar hold and the kickoff agenda this afternoon.\n\nSarah`
                    : ''
                }
                placeholder="Reply…"
              />
    ...
    <>
      <div className="module-head">
        <div>
          ...
          <div className="module-sub">
            ${(total / 1000).toFixed(1)}k across {D.DEALS.length} deals · forecasting $
            {((total * 0.42) / 1000).toFixed(1)}k closed in Q2
          </div>
        </div>
        <div className="module-toolbar">
          <button className="btn sm">
              ...
              <div key={stage.id} className="kanban-col">
                <div className="kanban-col-head">
                  ...
                  <span className="kanban-col-meta">
                    {deals.length} · ${(sum / 1000).toFixed(1)}k
                  </span>
                </div>
                {deals.map((d) => {
  ...
  <>
    <div className="module-head">
      <div>
        ...
        <div className="module-sub">
          {D.PROJECTS.filter((p) => p.status === 'active').length} active · 2 due this week
        </div>
      </div>
      <div className="module-toolbar">
        <button className="btn sm">
          <Icon name="layout" size={14} /> Grid
        </button>
        <button className="btn sm">
          <Icon name="list" size={14} /> List
        </button>
        <button className="btn sm">
          <Icon name="gantt" size={14} /> Timeline
        </button>
        <button className="btn primary sm">
            ...
            <div key={p.id} className="project-card">
              ...
              <div className="project-card-foot">
                <span className="avatar-stack">
                  {p.team.map((u) => (
                    <Avatar key={u} name={userById(u).name} size={22} color={userById(u).color} />
                  ))}
                </span>
